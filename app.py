...
def process_chunk(audiosr, chunk, sr, guidance_scale, ddim_steps, is_last_chunk=False, target_length=None):
    ...
    try:
        ...
        # IMPORTANT: Trim the output to match the original input length
        # AudioSR pads input to multiples of 5.12 seconds, which causes duration drift
        # Since both input and output are at 48kHz, output samples should equal input samples
        ...
        # For the last chunk, optionally trim further based on actual audio content
...
def process_audio_channel(audiosr, audio_channel, sr, guidance_scale, ddim_steps):
    ...
    for i in range(num_chunks):
        ...
        if i > 0:
            ...
            # Since we now trim output to match input length, the overlap size stays the same
